...
{
    public sealed class ProjectionExpressionResolver : IProjectionExpressionResolver, IProjectionExpressionBaseResolver
    {
        ...
        private readonly static ConcurrentDictionary<MemberInfo, LambdaExpression> _expressionBaseCache = new();
        ...
        public LambdaExpression FindGeneratedExpression(MemberInfo projectableMemberInfo,
            ...
            => _expressionCache.GetOrAdd(projectableMemberInfo, static (mi, a) => ResolveExpressionCore(mi, false, a),
                projectableAttribute);

        public LambdaExpression FindGeneratedBaseExpression(MemberInfo projectableMemberInfo,
            ProjectableAttribute? projectableAttribute = null)
            => _expressionBaseCache.GetOrAdd(projectableMemberInfo, static (mi, a) => ResolveExpressionCore(mi, true, a),
                projectableAttribute);
            ...
            ProjectableAttribute? projectableAttribute)
        {
            ...
            var expression = GetExpressionFromGeneratedType(projectableMemberInfo, isBase);
        ...
        private static LambdaExpression? GetExpressionFromGeneratedType(MemberInfo projectableMemberInfo, bool isBase)
        {
            var declaringType = projectableMemberInfo.DeclaringType ?? throw new InvalidOperationException("Expected a valid type here");

            // Fast path (isBase=false): check the per-assembly static registry (generated by source generator).
            ...
            LambdaExpression? registeredExpr;
            if (!isBase)
            {
                var registry = GetAssemblyRegistry(declaringType.Assembly);
                registeredExpr = registry?.Invoke(projectableMemberInfo);
            }
            else
            {
                registeredExpr = null; // We don't have a registry for base expressions for now
            }
            ...
            return registeredExpr ??
                   ...
                   FindGeneratedExpressionViaReflection(projectableMemberInfo, isBase);
        }
        ...
        private readonly static ConcurrentDictionary<MemberInfo, LambdaExpression> _reflectionBaseCache = new();
        ...
        {
            return FindGeneratedExpressionViaReflection(projectableMemberInfo, false);
        }
        private static LambdaExpression? FindGeneratedExpressionViaReflection(MemberInfo projectableMemberInfo, bool isBase)
        {
            var result = (isBase ? _reflectionBaseCache : _reflectionCache).GetOrAdd(projectableMemberInfo,
                mi => BuildReflectionExpression(mi, isBase) ?? _reflectionNullSentinel);
        ...
        /// </para>
        /// </summary>
        private static LambdaExpression? BuildReflectionExpression(MemberInfo projectableMemberInfo, bool isBase)
        {
            ...
            if (isBase)
                generatedContainingTypeName = generatedContainingTypeName + "_Base";
